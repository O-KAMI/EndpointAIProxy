...
function Assert-ProductionMsi([string]$Path) {
  ...
  try {
    ...
    $sequence = @{}
    foreach ($name in @('PreflightLegacy','InstallValidate','SnapshotConfiguration','InstallInitialize','RemoveExistingProducts','RollbackConfiguration','QuiesceLegacy','InstallServices','ProvisionProductionControl','StartServices','VerifyInstalledService','CommitConfiguration')) {
      $sequence[$name] = [int](Read-Cell "SELECT ``Sequence`` FROM ``InstallExecuteSequence`` WHERE ``Action``='$name'")
    }
    $ordered = @('PreflightLegacy','InstallValidate','SnapshotConfiguration','InstallInitialize','RemoveExistingProducts','RollbackConfiguration','QuiesceLegacy','InstallServices','ProvisionProductionControl','StartServices','VerifyInstalledService','CommitConfiguration')
