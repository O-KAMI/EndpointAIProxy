...
spec = importlib.util.spec_from_file_location("update_023", Path(__file__).with_name("update-server-0.1.23.py"))
...
def configure(monkeypatch, tmp_path):
    old, older, new = [tmp_path / name for name in ("old022", "old021", "new023")]
    old.mkdir()
    older.mkdir()
