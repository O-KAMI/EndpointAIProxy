...
CANDIDATES = (Path("/app/EndpointAIDLP-Server-0.1.21"), Path("/app/EndpointAIDLP-Server-0.1.20"))
NEW = Path("/app/EndpointAIDLP-Server-0.1.22")
ARCHIVE = "EndpointAIDLP-Server-0.1.22-update.zip"


def state_path():
    return NEW.parent / ".EndpointAIDLP-Server-0.1.22-upgrade.json"
...
def source_directory(explicit=None):
    if explicit is not None:
        ...
        require(len(active) == 1, "无法唯一识别运行的 0.1.20/0.1.21；请使用 --old-dir 指定真实服务目录。")
        old = active[0]
...
def main():
    ...
    os.umask(0o077)
    lock_path = NEW.parent / ".EndpointAIDLP-Server-0.1.22-upgrade.lock"
    ...
    with lock_path.open("a") as lock:
        ...
        if args.action == "status":
            ...
            print("0.1.22 运行中：" + str(bool(processes(NEW))))
            return
